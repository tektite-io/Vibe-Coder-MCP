...
export async function processWithSequentialThinking(userPrompt, config, systemPrompt) {
    ...
    while (currentThought.next_thought_needed && thoughts.length < MAX_SEQUENTIAL_THOUGHTS) {
        ...
        let nextThought = null; // Initialize as null, use Zod type
 ...
 */
function extractFallbackThoughtText(rawContent) {
    if (!rawContent) {
...
prompt, systemPrompt, config, currentThoughtNumber // Added parameter
) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try { // Inner try for each attempt
            ...
            if (response.data.choices && response.data.choices.length > 0) {
                ...
                // --- End Cleaning ---
                let parsedContent; // Keep this one
                let validationResult; // Define type for validation result
                try {
                    ...
                    if (validationResult.success) {
                        ...
                        throw new ValidationError(`Sequential thought validation failed: ${validationResult.error.message}`, validationResult.error.issues, // Pass ZodIssue[] directly
                    ...
                    if (parseOrValidationError instanceof SyntaxError) {
                        ...
                        throw new ValidationError(parseOrValidationError.message, parseOrValidationError.validationIssues, { ...parseOrValidationError.context, ...errorContext } // Combine contexts
                        );
                    }
                    else {
    ...
    if (lastError) {
        ...
        if ((lastError instanceof ParsingError || lastError instanceof ValidationError) && errorRawContent !== undefined) {
            ...
            `Sequential thinking aborted after ${maxRetries} attempts due to persistent LLM formatting error: ${lastError.message}`, errorRawContent, lastError // Pass the original ParsingError or ValidationError
            );
        }
        ...
        if (axios.isAxiosError(lastError)) {
            ...
            const apiError = new ApiError(apiMessage, status, { model: modelToUse, logicalTaskName }, // Include task name and model used
            axiosError);
            throw apiError;
        }
