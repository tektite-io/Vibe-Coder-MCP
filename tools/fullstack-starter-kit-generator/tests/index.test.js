...
import { generateFullstackStarterKit, initDirectories } from '../index.js';
  ...
  `;
    ...
    beforeEach(() => {
        ...
        vi.spyOn(researchHelper, 'performResearchQuery')
            ...
            if (query.includes('technology stack'))
                return mockResearchResults[0];
            if (query.includes('best practices'))
                return mockResearchResults[1];
            if (query.includes('development tooling'))
        ...
        }); // Use 'as any' to bypass complex Zod type issues in mock
    ...
    it('should validate JSON output, save it, and generate setup scripts', async () => {
        ...
        expect(schema.starterKitDefinitionSchema.safeParse).toHaveBeenCalledWith(expect.objectContaining({ ...mockParsedValidMainParts, directoryStructure: expect.any(Array) }));
        // Verify the definition file was saved
        expect(fs.writeJson).toHaveBeenCalledWith(expect.any(String), // path
        ...
        expect(scripts.generateSetupScripts).toHaveBeenCalledWith(expect.objectContaining({
            projectName: "test-project",
            description: "A test project"
        }));
        ...
        const shCallIndex = writeFileCalls.findIndex(call => typeof call[0] === 'string' && call[0].endsWith('.sh'));
    ...
    it('should return error on invalid JSON format', async () => {
        ...
        if (result.errorDetails &&
            ...
            const context = result.errorDetails.context; // Assert context as object
