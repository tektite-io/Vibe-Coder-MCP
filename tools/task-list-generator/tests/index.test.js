...
`;
    ...
    beforeEach(() => {
        ...
        vi.spyOn(researchHelper, 'performResearchQuery')
            ...
            if (query.includes('estimation'))
                return mockResearchResults[1];
            if (query.includes('team structure'))
        ...
        vi.spyOn(llmHelper, 'performDirectLlmCall')
            ...
            if (logicalTaskName === 'task_list_decomposition') {
                if (prompt.includes('T-101'))
                    return mockGeneratedSubTasksT101;
                if (prompt.includes('T-201'))
                    return mockGeneratedSubTasksT201;
            }
    ...
    it('should handle research failures gracefully', async () => {
        ...
        expect(llmHelper.performDirectLlmCall).toHaveBeenCalledWith(expect.any(String), // prompt
        expect.any(String), // system prompt
        mockConfig, 'task_list_initial_generation' // logical task name
        );
...
`; // Renamed this variable for clarity
        ...
        vi.mocked(llmHelper.performDirectLlmCall)
            ...
            if (logicalTaskName === 'task_list_decomposition') {
                if (prompt.includes('T-101'))
                    return consistentMockSubTasksT101;
                if (prompt.includes('T-201'))
                    return consistentMockSubTasksT201;
            }
            return 'Default mock LLM response';
        });
