 ...
 */
export function formatBackgroundJobInitiationResponse(jobId, toolName, message, context) {
    const sessionId = context?.sessionId || 'unknown-session';
    const transportType = context?.transportType || 'unknown';
    // Create a standardized job status message
    const statusMessage = createJobStatusMessage(jobId, toolName, JobStatus.PENDING, message, 0, // 0% progress
    Date.now(), Date.now());
