...
export class VueHandler extends BaseLanguageHandler {
     ...
     */
    isTemplateElement(node, sourceCode) {
        try {
     ...
     */
    isStyleElement(node, sourceCode) {
        try {
            const tagNameNode = node.childForFieldName('tag_name');
     ...
     */
    isInComputedSection(node, sourceCode) {
        try {
            let current = node;
     ...
     */
    isInWatchSection(node, sourceCode) {
        try {
            let current = node;
            while (current.parent) {
                current = current.parent;
     ...
     */
    isInMethodsSection(node, sourceCode) {
        try {
            let current = node;
            while (current.parent) {
                current = current.parent;
     ...
     */
    isScriptElement(node, sourceCode) {
        try {
            const tagNameNode = node.childForFieldName('tag_name');
     ...
     */
    findScriptNode(node, sourceCode) {
        try {
            // Find the script element
            for (let i = 0; i < node.childCount; i++) {
                const child = node.child(i);
