...
export class HtmlHandler extends BaseLanguageHandler {
     ...
     */
    hasStylesheetAttribute(node, sourceCode) {
        try {
     ...
     */
    findAttribute(attributesNode, name, sourceCode) {
        try {
     ...
     */
    getAttributeValue(attributeNode, sourceCode) {
        try {
     ...
     */
    extractFormName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractInputName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractLinkName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractContainerName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractHeadingName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractListName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    extractTableName(node, sourceCode) {
        try {
            const startTagNode = node.childForFieldName('start_tag');
            if (startTagNode) {
                const attributesNode = startTagNode.childForFieldName('attributes');
     ...
     */
    findMetaDescription(node, sourceCode) {
        try {
            // Find the head element
            let headNode = null;
            for (let i = 0; i < node.childCount; i++) {
