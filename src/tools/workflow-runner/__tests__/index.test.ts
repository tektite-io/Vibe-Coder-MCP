// src/tools/workflow-runner/__tests__/index.test.ts
...
describe('runWorkflowTool (Async)', () => {
    ...
    it('should return job ID and call executeWorkflow in background', async () => {
        ...
        expect(initialResult.content[0]?.text).toContain(`Job started: ${mockJobId} (Workflow 'myFlow' Execution)`);
    ...
    it('should set job result with formatted successful workflow output', async () => {
         ...
         expect(finalCallResult.content[0]?.text).toContain(`- finalMsg: All done!`);
    });
    ...
    it('should set job result with formatted failed workflow output', async () => {
         ...
         expect((finalCallResult.errorDetails as any)?.message).toBe(`MCP error -32603: ${mockFailResult.error?.message}`);
         expect((finalCallResult.errorDetails as any)?.data).toEqual(mockFailResult.error?.details);
    });
     ...
     it('should set job to FAILED if executeWorkflow itself throws an unexpected error', async () => {
          ...
          expect(errorDetails?.name).toBe('McpError');
     });
});
